...
@click.option('--output', '-o', type=click.Path(path_type=Path),
              help='Output file path (default: stdout or ragbrain-dump-<timestamp>.json)')
...
def dump_database(
    ...
    Examples:
        ...
        # Dump to a file
        ragbrain dump -o backup.json

        # Dump specific namespace
        ragbrain dump -n books/fiction -o fiction.json

        # Dump as JSON Lines (one record per line)
        ragbrain dump -f jsonl -o backup.jsonl

        # Include vectors (warning: large file)
        ragbrain dump --include-vectors -o full-backup.json
    """
    from ragbrain.config import settings
    from ragbrain.providers.factories import VectorDBProviderFactory
    import ragbrain.providers.vectordb  # noqa - registers providers
    ...
    if output is None and fmt == 'json':
        ...
        if sys.stdout.isatty():
            ...
            output = Path(f"ragbrain-dump-{timestamp}.json")
