...
from ragbrain.providers.base import NamespaceProvider
    ...
    def create(
        ...
        with self._get_connection() as conn:
            try:
                conn.execute(
                    """
                    INSERT INTO namespaces (id, name, description, parent_id, metadata, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    (id, name, description, parent_id, metadata_json, now, now)
                )
                conn.commit()
