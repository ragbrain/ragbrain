...
import { queryKnowledge } from '../api/client.js'
...
const sources = computed(() => {
  ...
  if (synthesize.value && result.value.sources) {
    return result.value.sources
  }
  if (!synthesize.value && result.value.results) {
...
const handleQuery = async () => {
  ...
  try {
    ...
    const response = await queryKnowledge(query.value, synthesize.value, topK.value, ns, rerank.value)
